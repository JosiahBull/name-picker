...
test.describe('Analytics', () => {
	test('should update analytics correctly as user swipes', async ({
		...
		await page.waitForSelector('text=Total Swipes', { state: 'visible' });
		{
			...
			await expect(totalSwipesElement).toHaveText(
				EXPECTED_AFTER_ONE_LIKE.totalSwipes.toString(),
			);
		...
		await page.waitForSelector('text=Total Swipes', { state: 'visible' });
		{
			...
			await expect(totalSwipesElement).toHaveText(
				EXPECTED_AFTER_ONE_LIKE_ONE_PASS.totalSwipes.toString(),
			);
			await expect(likesElement).toHaveText(
				EXPECTED_AFTER_ONE_LIKE_ONE_PASS.likes.toString(),
			);
			await expect(matchesElement).toHaveText(
				EXPECTED_AFTER_ONE_LIKE_ONE_PASS.matches.toString(),
			);
		}
