...
test.describe('Navigation', () => {
  test('should navigate between all main pages', async ({ page }) => {
    ...
    await expect(page.locator('header [class*="MuiTypography-h6"]')).toContainText('Your Matches');
  ...
  test('should show proper page titles in app bar', async ({ page }) => {
    await loginAsJoe(page);
    
    // Home page
    await expect(page.locator('header [class*="MuiTypography-h6"]')).toContainText('Name Picker');
  ...
  test('should display user info in app bar', async ({ page }) => {
    await loginAsJoe(page);
